...
def show():
    clear_message = lsb.reveal(filename)
    if clear_message:
        messagebox.showinfo("Hidden Message", "Hidden Message:\n\n" + clear_message)

#save the image
def save():
    global secret
    secret.save("hidden.png")
    messagebox.showinfo("Success", "Image saved with hidden data!")

#icon
image_icon=PhotoImage(file="encode.png")
root.iconphoto(False,image_icon)

#logo
logo=PhotoImage(file="logo.png")
Label(root,image=logo,bg="#2f4155").place(x=10,y=0)
Label(root,text="Hide a secret text in an image",bg="#2d4155",fg="white",font="arial 25 bold").place(x=100,y=20)
